                ...
                <ol className="mt-5 space-y-3 border-t border-[#dcdfeb] pt-5">
                  {sections.map(({ id, heading }, index) => (
                    <li key={id}>
                      <a
                        href={`#${id}`}
                        className="flex gap-3 text-[15px] leading-6 text-[#2c3272] transition-colors hover:text-[#ed6929]"
                      >
                      ...
                      {body.map((block, index) =>
                        Array.isArray(block) ? (
                          <ul key={index} className="space-y-3 border-l-2 border-[#ed6929] pl-6">
                            {block.map((item) => (
                              <li key={item} className="text-base leading-8 text-[#2c3272]">
                                {item}
                              </li>
                            ))}
                          </ul>
                        ) : (
                          <p key={index} className="text-base leading-8 text-[#2c3272]">
                            {block}
                          </p>
                        )
                      )}
